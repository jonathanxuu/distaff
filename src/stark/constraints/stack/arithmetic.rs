...
pub fn enforce_kvalid(result: &mut [u128], old_stack: &[u128], new_stack: &[u128], op_flag: u128){
    ...
    // result.agg_constraint(0, op_flag, are_equal(kvalid_a, field::kvalid_a(old_stack[7], old_stack[6], old_stack[5], old_stack[4], old_stack[3], old_stack[2], old_stack[0], old_stack[1], old_stack[8])));
...
pub fn enforce_khash(result: &mut [u128], old_stack: &[u128], new_stack: &[u128], op_flag: u128){
    ...
    // console_log!("n is {:?},is between is {:?}, mul = {:?}",n,is_between,op_flag * are_equal(field::ZERO,is_between));

    if is_between == field::ZERO && (2 * n as usize) <= old_stack.len() {
        // console_log!("is_between is {:?},old stack is {:?},new stack is {:?}",is_between,old_stack,new_stack);
        for i in 0..n {
